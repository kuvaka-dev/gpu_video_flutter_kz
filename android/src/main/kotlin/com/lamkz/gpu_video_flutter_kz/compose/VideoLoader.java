        ...
        public void run() {
            ...
            if (cursor.moveToLast()) {
                do {
                    ...
                    String duration = cursor.getString(cursor.getColumnIndexOrThrow(projection[1]));
                    if (duration == null) duration = "0";
                    Log.d(TAG, "pick video from device duration = " + duration);

                    String width = cursor.getString(cursor.getColumnIndexOrThrow(projection[2]));
                    if (width == null) width = "0";
                    Log.d(TAG, "pick video from device width = " + width);

                    String height = cursor.getString(cursor.getColumnIndexOrThrow(projection[3]));
